...
class Restrict extends CI_Controller{
	...
	public function ajax_import_image() {
		...
		if (!$this->upload->do_upload("image_file")) {
			...
			if ($this->upload->data()["file_size"] <= 1024) {
				...
				$json["status"] = 0;
				$json["error"] = "Arquivo não deve ser mais que 1 MB!";
			}
		}
	...
	public function ajax_save_course() {
		...
		if (empty($data["course_name"])) {
			...
			if ($this->courses_model->is_duplicated("course_name", 
		...
		if (empty($data["course_duration"])) {
			...
			if (!($data["course_duration"] > 0 && $data["course_duration"] < 100)) {
				$json["error_list"]["#course_duration"] = "Duração do curso deve ser maior que 0 (h) 
				e menor que 100 (h).";
			}
		}
	...
	public function ajax_save_user() {
		...
		if (empty($data["user_login"])) {
			...
			if ($this->users_model->is_duplicated("user_login", 
		...
		if (empty($data["user_email"])) {
			...
			if($this->users_model->is_duplicated("user_email", $data["user_email"],
				$data["user_id"])) {
	...
	public function ajax_list_course() {
		...
		foreach ($courses as $course) {
			...
			if ($course->course_img) {
				$row[] = '<img src="'.base_url().$course->course_img.'" 
			...
			$row[] = '<div style="display: inline-block;">
						<button class="btn btn-secundary btn-edit-course" 
							course_id="'.$course->course_id.'" style="margin-bottom: 3px">
	...
	public function ajax_list_member() {
		...
		foreach ($team as $member) {
			...
			if ($member->member_photo) {
				$row[] = '<img src="'.base_url().$member->member_photo.'" 
					style="max-height: 100px; max-width: 100px;">';
			} else {
				$row[] = "";
			}
			...
			$row[] = '<div style="display: inline-block;">
						<button class="btn btn-secundary btn-edit-member"
							member_id="'.$member->member_id.'" style="margin-bottom: 3px">
							<i class="fa fa-edit"></i>
						</button>
	...
	public function ajax_list_user() {
		...
		foreach ($users as $user) {
			...
			$row[] = '<div style="display: inline-block;">
						<button class="btn btn-secundary btn-edit-user" 
							user_id="'.$user->user_id.'" style="margin-bottom: 3px">
							<i class="fa fa-edit"></i>
						</button>
